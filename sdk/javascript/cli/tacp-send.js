...
async function main() {
  ...
  if (options.message) {
    try {
      messageData = JSON.parse(options.message);
    } catch (err) {
      console.error(`Error: Invalid JSON in --message: ${err.message}`);
      process.exit(EXIT_INVALID_ARGS);
    }
  } else if (options.input && options.input !== '-') {
    try {
      const inputContent = fs.readFileSync(options.input, 'utf8').trim();
      messageData = inputContent ? JSON.parse(inputContent) : {};
    } catch (err) {
      if (!options.quiet) {
    ...
    try {
      ...
      if (stdinData) {
        messageData = JSON.parse(stdinData);
      } else {
