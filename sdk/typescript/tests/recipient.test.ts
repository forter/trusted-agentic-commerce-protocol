...
describe("TACRecipient - Message Processing", () => {
  ...
  let recipientKeys: jose.GenerateKeyPairResult<jose.KeyLike>;
  let sender: TACSender;
  let senderKeys: jose.GenerateKeyPairResult<jose.KeyLike>;
  ...
  describe("Message Validation", () => {
    ...
    it("should handle missing TAC message (null)", async () => {
      const result = await recipient.processTACMessage(null as any);
    ...
    it("should handle missing TAC message (undefined)", async () => {
      const result = await recipient.processTACMessage(undefined as any);
  ...
  describe("Decryption", () => {
    it("should successfully decrypt with correct private key", async () => {
      ...
      const testData = {
        user: {
          ...
          // preferences: ["comfort", "durability"], // Remove this as it doesn't match schema
        },
        session: {
          ipAddress: "192.168.1.1",
        },
        timestamp: Date.now(), // Move this out of session since it's not part of the Session schema
        preferences: ["comfort", "durability"], // Move to custom field
      };
    ...
    it("should fail with wrong private key", async () => {
      ...
      const wrongRecipient = new TACRecipient({
        domain: "recipient.com",
        privateKey: wrongRecipientKeys.privateKey as any,
      });
      ...
      (sender as any).fetchJWKS = async () => [recipientJWK];
      (wrongRecipient as any).fetchJWKS = async () => [senderJWK];
    ...
    it("should fail when no private key loaded", async () => {
      ...
      const emptyRecipient = new TACRecipient({
        domain: "recipient.com",
        privateKey: recipientKeys.privateKey as any, // Will be cleared below
      });

      // Manually clear the private key to simulate missing key
      (emptyRecipient as any).privateKey = null;

      const recipientJWK = await recipient.getPublicJWK();
      const senderJWK = await sender.getPublicJWK();

      (sender as any).fetchJWKS = async () => [recipientJWK];
      (emptyRecipient as any).fetchJWKS = async () => [senderJWK];
  ...
  describe("Signature Verification", () => {
    ...
    it("should fail with invalid signature", async () => {
      ...
      (recipient as any).fetchJWKS = async () => [wrongSenderJWK];
    ...
    it("should handle wrong sender key", async () => {
      ...
      (recipient as any).fetchJWKS = async () => [differentSenderJWK];
    ...
    it("should handle JWKS fetch failure", async () => {
      ...
      (sender as any).fetchJWKS = async () => [recipientJWK];
      (recipient as any).fetchJWKS = async () => {
  ...
  describe("Multi-Recipient Messages", () => {
    it("should extract correct data from multi-recipient message", async () => {
      ...
      const recipient2 = new TACRecipient({
        ...
        privateKey: recipient2Keys.privateKey as any,
      });
      ...
      (recipient as any).fetchJWKS = async () => [senderJWK];
      (recipient2 as any).fetchJWKS = async () => [senderJWK];
      ...
      assert.strictEqual((result1.data as any).secret, "recipient1-secret");
      assert.strictEqual((result1.data as any).user.email.address, "user1@example.com");

      assert.strictEqual((result2.data as any).secret, "recipient2-secret");
      assert.strictEqual((result2.data as any).user.email.address, "user2@example.com");
    });
    ...
    it("should handle large number of recipients", async () => {
      ...
      recipients.forEach(({ instance }) => {
        (instance as any).fetchJWKS = async () => [senderJWK];
      });
      ...
      for (const { domain, instance } of recipients) {
        ...
        assert.strictEqual((result.data as any).id, expectedId);
        assert.strictEqual((result.data as any).data, `data for recipient ${expectedId}`);
      }
    });
    ...
    it("should ensure recipient data isolation", async () => {
      ...
      const vendor = new TACRecipient({
        domain: "vendor.com",
        privateKey: vendorKeys.privateKey as any,
      });
      ...
      (vendor as any).fetchJWKS = async () => [senderJWK];
      ...
      assert.strictEqual((merchantResult.data as any).customer_id, "cust_12345");
      assert.strictEqual((merchantResult.data as any).payment_method, "visa_ending_4242");
      assert.strictEqual((merchantResult.data as any).amount, 150.0);
      assert.strictEqual((merchantResult.data as any).merchant_secret, "TOP_SECRET_MERCHANT_DATA");
      assert.strictEqual((merchantResult.data as any).merchant_commission, 5.5);

      // Merchant should NOT see vendor data
      assert.strictEqual((merchantResult.data as any).shipping_address, undefined);
      assert.strictEqual((merchantResult.data as any).product_skus, undefined);
      assert.strictEqual((merchantResult.data as any).vendor_secret, undefined);
      assert.strictEqual((merchantResult.data as any).vendor_cost, undefined);
      ...
      assert.strictEqual((vendorResult.data as any).customer_id, "cust_12345");
      assert.strictEqual((vendorResult.data as any).shipping_address, "123 Main St, Secure City, SC 12345");
      assert.deepStrictEqual((vendorResult.data as any).product_skus, ["SECRET_SKU_001", "SECRET_SKU_002"]);
      assert.strictEqual((vendorResult.data as any).vendor_secret, "TOP_SECRET_VENDOR_DATA");
      assert.strictEqual((vendorResult.data as any).vendor_cost, 125.0);

      // Vendor should NOT see merchant data
      assert.strictEqual((vendorResult.data as any).payment_method, undefined);
      assert.strictEqual((vendorResult.data as any).amount, undefined);
      assert.strictEqual((vendorResult.data as any).merchant_secret, undefined);
      assert.strictEqual((vendorResult.data as any).merchant_commission, undefined);
    ...
    it("should prevent cross-recipient data leakage in large messages", async () => {
      ...
      for (const { domain, instance } of recipients) {
        ...
        // Should see own data
        assert.strictEqual((result.data as any).recipient_id, expectedData.recipient_id);
        assert.strictEqual((result.data as any).secret_data, expectedData.secret_data);
        assert.strictEqual((result.data as any).private_key_hint, expectedData.private_key_hint);
        assert.strictEqual((result.data as any).sensitive_amount, expectedData.sensitive_amount);
        ...
        for (const [otherDomain, otherData] of Object.entries(recipientData)) {
          if (otherDomain !== domain) {
            ...
            assert.notStrictEqual((result.data as any).secret_data, otherData.secret_data);
            assert.notStrictEqual((result.data as any).private_key_hint, otherData.private_key_hint);
            assert.notStrictEqual((result.data as any).sensitive_amount, otherData.sensitive_amount);
          }
        }
      }
    });
    ...
    it("should fail when recipient tries to decrypt another recipients message", async () => {
      ...
      const vendorResult = await vendor.processTACMessage(tacMessage);
      assert.strictEqual(vendorResult.valid, true);
      assert.strictEqual((vendorResult.data as any).vendor_only_data, "This should only be readable by vendor");
  ...
  describe("Error Recovery and Edge Cases", () => {
    ...
    it("should handle very large number of recipients in message", async () => {
      ...
      assert.strictEqual(result.valid, true);
      assert.strictEqual((result.data as any).data, "test");
    });
    ...
    it("should provide meaningful error messages", async () => {
      ...
      for (const { input, expectedError } of tests) {
        const result = await recipient.processTACMessage(input as any);
        assert.strictEqual(result.valid, false);
        assert.ok(
