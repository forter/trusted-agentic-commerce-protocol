...
async function decryptAndVerifyMessage(
  jwe: string,
  recipientPrivateKey: jose.KeyLike,
  senderPublicKey: jose.KeyLike | Uint8Array
) {
  ...
  const { payload } = await jose.jwtVerify(signedJWT, senderPublicKey as jose.KeyLike);
...
describe("TACSender - Message Generation", () => {
  ...
  describe("JWT Creation", () => {
    ...
    it("should set correct expiration time based on TTL", async () => {
      ...
      assert.ok((decrypted.payload.exp as number) >= expectedExpMin);
      assert.ok((decrypted.payload.exp as number) <= expectedExpMax);
    });
    ...
    it("should use second-precision timestamps", async () => {
      ...
      assert.strictEqual((decrypted.payload.iat as number) % 1, 0);
      assert.strictEqual((decrypted.payload.exp as number) % 1, 0);
    });
    ...
    it("should include various data structures in payload", async () => {
      ...
      const complexData = {
        user: {
          ...
          // preferences: ["Nike", "Adidas"], // Remove this as it doesn't match schema
          profile: {
            age: 30,
            location: "US",
            active: true,
        ...
        timestamp: Date.now(), // Move this out of session
        metadata: null,
        numbers: [1, 2, 3.14, -5],
        flags: { premium: true, verified: false },
        preferences: ["Nike", "Adidas"], // Move to custom field
      };
  ...
  describe("Multi-Recipient Encryption", () => {
    ...
    it("should handle multiple recipients (2 recipients)", async () => {
      ...
      const kids = message.recipients.map((r: any) => r.kid);
    ...
    it("should handle many recipients (10+ recipients)", async () => {
      ...
      const recipientKeys: Record<string, jose.GenerateKeyPairResult<jose.KeyLike>> = {};
    ...
    it("should ensure recipient data isolation", async () => {
      ...
      for (const recipient of message.recipients) {
        if (recipient.kid === "recipient1.com") {
          const senderPublicKey = await jose.importJWK(await jose.exportJWK(senderKeys.publicKey));
          const decrypted = await decryptAndVerifyMessage(recipient.jwe, recipient1Keys.privateKey, senderPublicKey);
          assert.strictEqual((decrypted.payload.data as any).secret, "recipient1-secret-data");
          assert.strictEqual((decrypted.payload.data as any).shared, "common-data");
          ...
          const senderPublicKey2 = await jose.importJWK(await jose.exportJWK(senderKeys.publicKey));
          const decrypted = await decryptAndVerifyMessage(recipient.jwe, recipient2Keys.privateKey, senderPublicKey2);
          assert.strictEqual((decrypted.payload.data as any).secret, "recipient2-secret-data");
          assert.strictEqual((decrypted.payload.data as any).shared, "common-data");
  ...
  describe("Large Data Handling", () => {
    it("should handle large payloads (1MB)", async () => {
      ...
      assert.strictEqual((decrypted.payload.data as any).largeField.length, 1024 * 1024);
    });
